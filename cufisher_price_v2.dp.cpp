...
*/
/*
DPCT1050:1: The template argument of the RNG engine could not be deduced. You need to update this code.
*/
void transport(int n, adept::BlockData<MyTrack> *block,
               oneapi::mkl::rng::device::philox4x32x10<dpct_placeholder /*Fix the vec_size manually*/> *states,
...
void select_process(adept::BlockData<MyTrack> *block, Scoring *scor, curandState_t *states,
                               Queue_t *queues[])
{
...
void process_eloss(int n, adept::BlockData<MyTrack> *block, Scoring *scor, curandState_t *states,
                              Queue_t *queue)
{
  int particle_index;
...
void process_pairprod(int n, adept::BlockData<MyTrack> *block, Scoring *scor, curandState_t *states,
                                 Queue_t *queue)
{
  int particle_index;
  for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < n; i += blockDim.x * gridDim.x) {
...
void init(curandState_t *states)
{
