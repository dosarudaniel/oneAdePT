...
int runSimulation()
{
  ...
  fillArray.Run(
      [](int thread_id, Atomic_int *index, int *array) { array[thread_id] = (*index)++;
      },                                                   // lambda being run
      32,                                                  // number of elements
      {sycl::range<3>(1, 1, 2), sycl::range<3>(1, 1, 16)}, // run with 2 block of 16 threads (if backend=CUDA)
  ...
  generate.Run(generateFunc, capacity, {sycl::range<3>(1, 1, 0), sycl::range<3>(1, 1, 0)}, blockT);
  ...
  selector.Run(selectTrackFunc, blockT->GetNused(), {sycl::range<3>(1, 1, 0), sycl::range<3>(1, 1, 0)}, blockT, 2,
               selection1);
  ...
  process_tracks.Run(elossFunc, selection1->size(), {sycl::range<3>(1, 1, 1000), sycl::range<3>(1, 1, 32)}, selection1,
                     blockT, blockH);
